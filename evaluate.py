...
def evaluate(model_config: str, ckpt_path: str, dataset_names: list[str], limit_batches: int = None):
    ...
    for ds_name in dataset_names:
        print(f"\n--- Testing on: {ds_name} ---")
        datamodule = Jiang2018DataModule() if ds_name == "Jiang2018" else VSIDataModule(dataset_name=ds_name)
        ...
        if test_results:
            ...
            all_results.append({
                ...
                "architecture": Path(model_config).stem, # Log the config name as the arch
                "dataset": ds_name,
                "mae": mae,
                "std": std,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, required=True, help="Path to the model YAML config used for training.")
    parser.add_argument("--ckpt", type=str, required=True, help="Path to the checkpoint file.")
    parser.add_argument("--datasets", nargs="+", default=["ZStack_HE", "ZStack_IHC", "Jiang2018"])
    parser.add_argument("--limit_batches", type=int, default=None, help="Limit number of test batches for a quick run.")
