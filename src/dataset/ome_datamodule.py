...
class OMEDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        master_index_path = config.get_master_index_path(self.dataset_name, patch_size=self.patch_size)
        if not master_index_path.exists():
            print(f"\n[ERROR] Required master index for {self.dataset_name} is missing.")
            print(f"Please run: python src/dataset/ome_prep/preprocess.py --dataset {self.dataset_name}")
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not split_path.exists():
            ...
            print(f"Warning: Split file {split_path} missing. Using all data for train.")
            splits = {"train_pool": [s.name for s in master_index.file_registry], "test": []}
        else:
    ...
    def _filter_index(self, master_index: MasterIndex, files: list[str]) -> ProcessedIndex:
