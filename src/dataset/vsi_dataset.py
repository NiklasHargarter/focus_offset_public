...
class VSIDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Map local index to (patch_idx, z_idx) based on flattening stride
        patch_idx = local_idx // num_z
        z_level = local_idx % num_z

        x, y, best_z = patches[patch_idx]

        # Lazy load slide (multiprocessing safe per-worker)
        ...
        try:
            ...
            if self.transform:
                ...
                # Default: [H, W, C] (uint8) -> [C, H, W] (float32) in [0, 1]
