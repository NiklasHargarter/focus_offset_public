...
def get_vsi_transforms(mode: Literal["train", "val"]):
    if mode == "train":
        return A.Compose(
            [
                A.D4(p=1.0),
...
def get_vsi_dataloaders(
    ...
    train_ds = get_vsi_dataset(
        index_df=train_index,
        slide_dir=slide_dir,
        downsample=downsample,
        transform=get_vsi_transforms("train"),
    )
    val_ds = get_vsi_dataset(
        index_df=val_index,
        slide_dir=slide_dir,
        downsample=downsample,
        transform=get_vsi_transforms("val"),
    )
