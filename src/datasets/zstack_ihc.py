"""ZStack IHC dataset configuration and public API."""

import argparse
import pandas as pd
from src.datasets.vsi.config import CACHE_ROOT, DATA_ROOT, SPLITS_ROOT
from src.datasets.vsi.dataset import get_vsi_dataset
from src.datasets.vsi.loader import get_vsi_dataloaders, get_vsi_test_loader
from src.datasets.vsi.prep.download import download_vsi_dataset
from src.datasets.vsi.prep.indexing import index_vsi_dataset


# Configuration
NAME = "ZStack_IHC"
FULL_NAME = "ZStack_IHC"
...
def get_ihc_dataset(index_df: pd.DataFrame | None = None):
    if index_df is None:
...
# Prep Commands API
def download_zstack_ihc(workers: int = 4):
...
def preprocess_zstack_ihc(workers: int | None = None, dry_run: bool = False):
    params = {
        "downsample": DOWNSAMPLE,
        "cov": COV,
        "patch_size": PATCH_SIZE,
...
if __name__ == "__main__":
    ...
    if args.download:
        download_zstack_ihc(workers=args.workers)
    if args.preprocess:
        preprocess_zstack_ihc(workers=args.workers, dry_run=args.dry_run)
