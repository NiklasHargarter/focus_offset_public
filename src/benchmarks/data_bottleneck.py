...
def main():
    ...
    dummy_imgs = torch.randn(BS, 3, 224, 224).cuda().to(memory_format=torch.channels_last).bfloat16()
    ...
    with torch.amp.autocast("cuda", dtype=torch.bfloat16):
        with torch.no_grad():
            ...
            for _ in range(10): 
                batch = next(it)
                gpu_img = batch[0].cuda().to(memory_format=torch.channels_last).bfloat16()
                _ = model(gpu_img)
