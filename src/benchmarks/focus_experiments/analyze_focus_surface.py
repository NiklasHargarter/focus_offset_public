...
def analyze_surface(vsi_path: Path, output_dir: Path, downscales: list[int]):
    ...
    # 1. Detect Tissue once using 4x as a stable mask reference
    ...
    for ds in downscales:
        ...
        for fps in patch_sizes:
            ...
            # 2. Generate Z-Map for this size/downscale
            # create_focal_map returns a grid of best Z-indices for the whole slide.
            z_grid = create_focal_map(scene, focus_patch_size=fps, downscale=ds)
            
            # 3. Filter for tissue and collect points
            points = []
