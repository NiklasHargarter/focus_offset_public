...
- **Strict Sub-Sampling & Fast Dry-Runs:** The raw HE and IHC datasets consist of hundreds of gigabytes of dense biological imaging. A naive full `for`-loop across the dataset will consume immense compute capital and hang the execution flow for hours. Therefore, during initial script development, execution, and verification, developers and AI Agents are strictly mandated to explicitly hardcode temporary sub-sampling bounds (e.g., `limit_slides=2`, `limit_patches=10`) directly into their loops or utilize "test-mode" arguments to guarantee execution loops finish aggressively fast.
